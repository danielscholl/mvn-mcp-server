"""Maven API service for Maven MCP Server.
...
import requests
from mcp.server.fastmcp.exceptions import ResourceError, ValidationError

from maven_mcp_server.shared.data_types import ErrorCode
from maven_mcp_server.services.cache import MavenCache
...
class MavenApiService:
    ...
    def get_latest_version(
        ...
        try:
            ...
            # Check if we have a release version
            if metadata.get("release_version"):
                # If classifier is specified, verify the artifact exists
                if classifier:
                    release_exists = self.check_artifact_exists(
                        group_id, artifact_id, metadata["release_version"], 
                        packaging, classifier
                    )
                    if release_exists:
                        return metadata["release_version"]
                else:
                    return metadata["release_version"]
            
            # If no release version or it doesn't have the classifier,
            # fall back to latest version
            if metadata.get("latest_version"):
                # If classifier is specified, verify the artifact exists
                if classifier:
                    latest_exists = self.check_artifact_exists(
                        group_id, artifact_id, metadata["latest_version"], 
                        packaging, classifier
                    )
                    if latest_exists:
            ...
            # If no latest/release version or they don't have the classifier,
            # try to find the latest from all versions
            ...
            raise ResourceError(
                f"No suitable version found for {group_id}:{artifact_id} with packaging {packaging} and classifier {classifier}",
